...
async def share_file_android(file_path, app_impl, main_window, java=None):
    ...
    Args:
        ...
        main_window: Haupt-Fenster für Dialoge

    Returns:
        bool: True wenn erfolgreich
    """
    # Konvertiere zu Path falls nötig
    ...
    # Versuche Android-Klassen zu importieren
    try:
    ...
    try:
        ...
        # Erstelle ContentValues für MediaStore
        values = contentvalues()
        values.put("_display_name", file_path.name)
        values.put("mime_type", "application/vnd.oasis.opendocument.spreadsheet")

        # Verwende Downloads Collection (öffentlich zugänglich)
        collection_uri = mediastore.Downloads.EXTERNAL_CONTENT_URI

        # Füge Datei zu MediaStore hinzu
        file_uri = content_resolver.insert(collection_uri, values)

        if file_uri is None:
            raise Exception("Konnte URI nicht erstellen")
        ...
        # Erstelle Share Intent mit content:// URI
        share_intent = Intent(Intent.ACTION_SEND)
        share_intent.setType("application/vnd.oasis.opendocument.spreadsheet")
        share_intent.putExtra(Intent.EXTRA_STREAM, file_uri)
        share_intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)

        # Erstelle Chooser
        chooser = Intent.createChooser(share_intent, "ODS-Datei teilen mit...")
